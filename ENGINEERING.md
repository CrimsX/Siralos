...
At this stage the read-only workspace tools are available; Git inspection is available only on hosts with an enforcing sandbox backend; workspace mutations, `/undo`, and command execution are intentionally unavailable; Godot discovery and static project inspection are available, but Godot engine probing is intentionally unavailable (see below).

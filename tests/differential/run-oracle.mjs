...
const INSTRUCTIONS_PROBE = join(HERE, "probes", "instructions-oracle.mjs");
const KNOWLEDGE_PROBE = join(HERE, "probes", "knowledge-oracle.mjs");
const REFERENCE_IDENTITY_PROBE = join(HERE, "probes", "reference-identity-oracle.mjs");
...
const CI_ARTIFACT_DIGEST_PROBE = join(
  HERE,
  "probes",
  "content-identity-artifact-digest-oracle.mjs",
);
const CI_CONTRACT_DIGEST_PROBE = join(
  HERE,
  "probes",
  "content-identity-contract-digest-oracle.mjs",
);
...
export function runScenario(scenario, root) {
  ...
  if (scenario.subject === "instructions-resolution") {
    return {
      scenarioId: scenario.id,
      subject: scenario.subject,
      outcome: SCENARIO_OUTCOME.COMPLETED,
      result: runWorkspaceProbe(INSTRUCTIONS_PROBE, scenario.subject, scenario.input),
    };
  }
  if (scenario.subject === "knowledge-revisions") {
    return {
      ...
      subject: scenario.subject,
      outcome: SCENARIO_OUTCOME.COMPLETED,
      result: runWorkspaceProbe(KNOWLEDGE_PROBE, scenario.subject, scenario.input),
    };
  }
  if (scenario.subject === "reference-identity") {
    return {
      scenarioId: scenario.id,
      subject: scenario.subject,
      outcome: SCENARIO_OUTCOME.COMPLETED,
      result: runWorkspaceProbe(REFERENCE_IDENTITY_PROBE, scenario.subject, scenario.input),
    };
  }
  if (scenario.subject === "research-policy") {
    return {
      scenarioId: scenario.id,
      subject: scenario.subject,
      outcome: SCENARIO_OUTCOME.COMPLETED,
      result: runWorkspaceProbe(RESEARCH_POLICY_PROBE, scenario.subject, scenario.input),
    };
  }
  if (scenario.subject === "planning-runtime") {
    return {
      scenarioId: scenario.id,
      subject: scenario.subject,
      outcome: SCENARIO_OUTCOME.COMPLETED,
      result: runWorkspaceProbe(PLANNING_RUNTIME_PROBE, scenario.subject, scenario.input),
    };
  }
  if (scenario.subject === "executor-brief") {
    return {
      scenarioId: scenario.id,
      subject: scenario.subject,
      outcome: SCENARIO_OUTCOME.COMPLETED,

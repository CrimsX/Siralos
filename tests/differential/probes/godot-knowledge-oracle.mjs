/**
 * godot-knowledge oracle probe (differential harness, ADR 0033,
 * Stage 3R R8).
 *
 * Spawned by the oracle runner with the scenario input JSON on stdin.
 * Executes knowledge scenarios against the REAL TypeScript reference
 * service (createGodotKnowledgeService over the fail-closed production
 * generation runner): status, refresh cancellation/unavailability,
 * input-ordered search validation, and lookup. Thin, bounded, no
 * engine; mirrors the record vocabulary of the Rust candidate arm in
 * crates/siralos-cli/src/harness.rs::godot_knowledge_record.
 */
import { readFileSync } from "node:fs";
import { createGodotKnowledgeService } from "../../../packages/adapters/src/godot/knowledge/godot-knowledge-service.js";
...
const service = createGodotKnowledgeService(dependencies);

async function cancelledRefresh() {
  const controller = new AbortController();
  controller.abort();
  try {
    return await service.refresh(controller.signal);
  } catch (error) {
...
switch (input.op) {
  case "status":
    outcome = await service.status();
    break;
  case "refresh":
    outcome = input.cancelled === true ? await cancelledRefresh() : await service.refresh();
    break;
  case "search": {
    ...
    if (result.status === "ready") {
      throw new Error("knowledge search cannot become ready without a base");
    }
    outcome = { status: result.status, message: result.message };
    break;
  }
  case "lookup": {
    const result = await service.lookup(input.symbol);
    if (result.status === "ready") {
      throw new Error("knowledge lookup cannot become ready without a base");
    }
    outcome = { status: result.status, message: result.message };
    break;
  }
  default:
    throw new Error(`unknown godot-knowledge op ${String(input.op)}`);
}

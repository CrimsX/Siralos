...
implementation is mechanical rather than exploratory. It ports nothing and
...
- `ModelEvent` union is closed and exactly three variants (provider.ts:12-25): `text_delta {text}`, `tool_call {callId, toolName, input}`, `completed`. Any other event shape is handled as a tool-call-shaped event and fails validation (empty id/name path).
...
- **Tool calls**: JSON round-tripped (`JSON.parse(JSON.stringify(input))`) before retention/execution so caller references and unknown fields never cross (provider-turn.ts:175-232; provider-protocol.test.ts:241-264). Empty call id or empty tool name → an `invalid` call with synthetic id `invalid-call-N` (provider-turn.ts:209-216); duplicate call id → `invalid` call "Duplicate tool call id: …" while the first occurrence still executes (provider-turn.ts:217-224; tool-loop.test.ts:179-198). The strict adapter collector instead fails the whole turn on duplicates (bounded-model-turn.ts:256-261) — both behaviors are contract and belong to different call sites.
...
| Rust owner            | `siralos-adapters`: file loading, bounded read, strict parse/validation; `siralos-core`: none beyond the existing doctor model                                                                                                                                                         |
...
| F6  | Empty call id/name                                                                                                               | provider stream   | `invalid` call (not failure)        | "Provider emitted a tool call with an empty call id or tool name."                                                               | n/a                        | no                     | failed result for synthetic id         | yes          |
| F7  | Duplicate call id (application)                                                                                                  | provider stream   | `invalid` call                      | "Duplicate tool call id: <id>."                                                                                                  | n/a                        | no                     | failed result for synthetic id         | yes          |
...
substring matching. F1-F5, F8, F10, F11, F13-F17 are retryable turn/tool
failures; F9, F18-F20, F25 are terminal for the prompt/startup; F23/F24 are
terminal at startup; F22 is not a failure. Every runtime failure leaves the
session usable; only startup failures prevent it. Stringly TypeScript errors
that should become typed Rust errors without changing observable behavior:
turn limit classes (F1), protocol violations (F2/F3/F5/F8), transcript
invalidity (F9), tool-result detach classes (F10), and config rule violations
(F23/F24).

## 5. Resource bounds

All bounds are enforced on **UTF-8 bytes**, not characters; the boundary is
inclusive (a value exactly at the limit is accepted; `>` triggers rejection).
...
The strict adapter collector accepts the same seven dimensions caller-supplied
and normalizes numeric budgets via clamp/floor with fallback
(bounded-model-turn.ts:82-93); "callers may choose a smaller assistant-text
limit, but every other dimension remains explicitly bounded".

### 5.2 Loop budgets

- Tool rounds: default 8, hard max 32, clamp [0,32] (application.ts:111-112, 132-137).
- Display input truncation: 200 chars + "…" (application.ts:130, 1027-1035).

### 5.3 Projection

- Working maximum 32,768 tokens; output tokens 4,096 (carried, unused in math).
...
- File ≤ 1 MiB (declared-size pre-check + bounded EOF-verified read); references ≤ 16; godot installations ≤ 16; installation id ≤ `GODOT_LIMITS.maxInstallationIdLength`; reviewProvider ≤ 128 chars, `^[A-Za-z0-9._-]+$`.
...
| provider-turn.duplicate-call-id                                           | provider-turn      | duplicate call id in one turn                                       | invalid-call + failed-result record (application semantics)              | required | *        | duplicate id handling                        |
| provider-turn.empty-id-name                                               | provider-turn      | empty call id / tool name                                           | invalid-call records                                                     | required | *        | empty-id/name handling                       |
...
`provider-turn.cancelled`, `provider-turn.result-detach.*`
(≈ 11 scenarios). Later scenarios for complete R7: the rest of
`provider-turn`, all `tool-loop`, `context-projection`,
`user-config`, and `cli-session` (planned ≈ 39 scenarios total; exact
counts finalized when each subject is implemented). Authority assumptions:
...
- **Configuration**: adapters own filesystem loading (canonical path, bounded
  EOF-verified read, symlink rejection); core owns the validation semantics
  where the doctor consumes them; CLI owns path selection and diagnostics
  rendering.
...
- Godot domain behavior (all `godot.*` tools and config semantics) — R8/R9.
...
```text
R1-R6 — Verified
R7 — Behavior extraction complete; implementation active (R7.1 next)
R8-R12 — Not due
```

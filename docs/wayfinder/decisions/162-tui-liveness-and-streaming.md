...
| M7 VISIBLE STREAMING (`bc16526`)           | The terminal is shared, the loop owns one non-blocking `draw_now`, the last context pane is cached, and `TuiSink` asks for a coalesced frame (33 ms) whenever a line lands.                                                                                                                                                                        | `sink_requests_a_coalesced_redraw_after_it_changes_the_transcript`                                                            |

...
| F6 STREAMING IS VISIBLE              | The TUI shares its terminal (`Rc<RefCell<Terminal>>`), owns one non-blocking `draw_now`, caches the last context pane, and the sink asks for a frame per accepted line, throttled to 16 ms.                                             | `sink_requests_a_coalesced_redraw_after_it_changes_the_transcript`                                                                                  |

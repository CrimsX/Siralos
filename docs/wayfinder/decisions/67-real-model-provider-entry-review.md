...
- **Q1 Scope** — approved: _"Add to ProfileRecord"_ — bounded optional `provider`/`model` in `ProfileRecord`, no model-router.
- **Q2 Credentials** — approved: _"Env only"_ — `env:` references only, never in portable config.
- **Q3 Network** — approved: _"Bounded Host-observed"_ — explicit, bounded, Host-observed with replay.
- **Q4 Tools** — approved: _"Host gate"_ — `ToolRegistry` gate before execute, untrusted until validated.
- **Q5 Ordering** — approved: _"Approve ordering"_ — 4-step ordered slice sequence.

...
Future-facing rows are interpreted under
[ADR 0036](../adr/0036-lean-product-composition-and-extension-model.md) (the
lean product, composition, and extension model); no duplicate RFC documents
are created merely to reflect new terminology.
...
| RFC-0004 | Event Journal                     | PARTIALLY OWNED       | ADR 0014 and ADR 0029 own the bounded pieces; the durable journal is not committed (ADR 0036); disposition in decision 174                                                                   |
...
| RFC-0008 | Domain API                        | PARTIALLY OWNED       | [ADR 0034](../adr/0034-godot-domain-host-boundary.md); Plugin/View/Domain contribution model per ADR 0036; package-facing API is Stage 3R R6                                                 |
| RFC-0009 | Domain Package Format             | BACKLOG / NOT DUE     | ADR 0036: explicit install, versioned, digestable; no marketplace or package manager is committed (permanent boundary); disposition in decision 174                                          |
...
| RFC-0011 | Error and Diagnostic Model        | PARTIALLY OWNED       | Typed result contracts exist (R5 diagnostics, typed outcomes); the migration this waited on is complete (decision 40), so the consolidation is unblocked: ticket 134 scopes it               |
| RFC-0012 | Persistence and Recovery          | PARTIALLY OWNED       | ADR 0006 and ADR 0031 own bounded pieces; durable persistence is not committed (ADR 0036) -- a permanent boundary (decision 174), and no ambient Memory subsystem                            |
...
| RFC-0018 | Package Trust and Supply Chain    | PARTIALLY OWNED       | SUPPLY_CHAIN.md and ADR 0034; the plugin ecosystem is not due (ADR 0036) and the licensing/cargo-vet decisions are owned by ticket 132                                                       |
| RFC-0019 | Tasks, Budgets, and Orchestration | OWNED BY ACCEPTED ADR | ADR 0014, ADR 0020, ADR 0022, ADR 0023, ADR 0031, ADR 0036 (orchestration is not a foundational layer; Runs are bounded executions)                                                          |
| RFC-0020 | Observability and Trace Model     | PARTIALLY OWNED       | ADR 0014/0029/0031 plus the live context metrics ring (decision 91) and the /context audit (decision 100); a durable runtime trace protocol needs the uncommitted persistence (decision 174) |

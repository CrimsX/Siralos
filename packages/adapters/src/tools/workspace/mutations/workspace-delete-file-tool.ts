...
export const WORKSPACE_DELETE_UNAVAILABLE_MESSAGE =
  "workspace.delete_file is unavailable: Node offers no directory-relative (openat/renameat) primitive, so a same-user process that swaps a parent or target at any instruction boundary can redirect pathname-based displacement and unlink outside the workspace. The operation fails closed before any write; it will become available when a mechanically identity-bound commit primitive exists.";
...
export function createWorkspaceDeleteFileTool(
  ...
  return {
    ...
    definition: {
      ...
      description:
        "Unavailable: Node cannot bind pathname-based deletion to a verified parent against a same-user adversary, so the operation fails closed before any write.",
      inputSchema: {
        type: "object",
        properties: {

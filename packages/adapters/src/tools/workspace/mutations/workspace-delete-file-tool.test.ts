...
describe("workspace.delete_file fail-closed availability", () => {
  it("is unavailable before any write, approval, or checkpoint", async () => {
    ...
    const tool = createWorkspaceDeleteFileTool(workspace.root, createMutationLock(), store);
    const prepared = await tool.prepare(
      { path: "obsolete.txt", expectedSha256: "f".repeat(64) },
      {},
    );

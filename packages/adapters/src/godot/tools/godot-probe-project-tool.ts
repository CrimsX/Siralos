 ...
 * Approval is one-time and binds to the prepared probe digest. When
 * execution is unavailable on the platform, preparation reports a typed
 * `unavailable` result and no approval is requested.
 */
export function createGodotProbeProjectTool(probe: GodotProjectProbe): PreparedProjectProbeTool {
  return {
    ...
    definition: {
      ...
      description:
        "Run one recovery-mode Godot editor probe against a disposable mirror of the current project: the source workspace is never opened by Godot, network is denied, secrets are removed, generated cache stays inside the mirror, and the mirror is destroyed afterwards. Requires one-time approval bound to the current project risk manifest. Reports unavailable when this platform cannot bind execution to the approved bytes.",

...
export function computeGodotRiskManifestDigest(
  ...
  return computeProbeSha256(canonicalizeJson(manifest));
}
...
export function computeGodotPreparedProbeDigest(parts: GodotPreparedProbeDigestParts): string {
  return computeProbeSha256(canonicalizeJson(parts));
}
 ...
 * digest under the approved digest.
 */
...
function computeProbeSha256(text: string): string {
  const digest = cryptoDigest(text);
  return digest;
}

function cryptoDigest(text: string): string {
  // Core is Node-free by architecture check, so the digest primitive is a

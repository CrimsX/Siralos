/**
 * Pure digest primitives for Node-free core digest computation.
 *
 * Core imports no Node modules (enforced by the architecture check), so
 * SHA-256 is a small pure implementation over platform globals (FIPS 180-4).
 * The canonical JSON serializer makes equal structures always produce equal
 * digests regardless of key order.
 */

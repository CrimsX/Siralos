...
async function main() {
  if (!godotPath || godotPath.trim().length === 0) {
    skip(
      "setup",
      "SIRALOS_TEST_GODOT is not set; live diagnostics conformance requires an explicit Godot editor executable.",
    );
    return;
  }
  ...
  const workRoot = await mkdtemp(join(tmpdir(), "siralos-diagnostics-conformance-"));

...
`npm run test:sandbox` runs fixed internal conformance probes against the real backend: inside-workspace reads and writes, outside-workspace write denial, denied-secret reads, denied host reads (existing home and system paths outside the approved surface), loopback, reserved private-network, and DNS network denial, Unix-socket denial, provider-secret absence, descendant-process confinement, output limits, timeout, and cancellation — plus validation-command probes under `validation-offline`: Node and npm scripts read fixtures but cannot write the workspace (root, child, grandchild, and npm script shell all denied), cannot reach outbound or loopback network, receive no provider credentials and no `NODE_OPTIONS`, npm pre/post hooks do not run, stdin is closed, output limits terminate the process, timeout and cancellation terminate descendants that ignore normal termination, no sandbox files appear in the workspace, and sandbox-private run-directory cleanup succeeds. The probes use temporary directories and fake secrets only. The command makes no public internet request, never elevates, and returns nonzero when an available backend violates a required boundary. Unavailable backends produce a loud skipped result, never a passing one, and probes that cannot execute (for example Unix sockets on an unsupported host) are reported as skipped, never as passed.

## Host-read boundary

Sandboxed processes have an explicit deny-by-default host-read boundary. Inside the sandbox, reads are allowed by default, so Solaris denies every user-data and shared-scratch region of the host explicitly and re-allows only the approved surface: the configured workspace, the Solaris-owned sandbox-private directories, and the smallest runtime/system paths required for tooling. On Windows this denies all user profiles; on macOS it denies `/Users`, `/private/var/folders`, temp and log regions, and mounted volumes; on Linux it denies `/home`, `/root`, `/tmp`, `/var` scratch regions, `/srv`, `/mnt`, and `/media`. "Deny the home directory" is never treated as "workspace-only": host paths beside the workspace, in temp, or under other users are denied reads by default. Network, loopback, private-network destinations, and Unix sockets are denied for offline execution, and unsupported isolation reports unavailable and refuses execution — there is no unrestricted fallback.

## Sandbox wrapper environment contract

The Sandbox Runtime wrapper returns the environment its wrapped invocation requires; Solaris merges exactly that explicit set into its minimal allowlisted child environment. `process.env` is never passed through wholesale: wrapper-only variables are added, collisions resolve to the Solaris-controlled value (the wrapper can never override protected variables such as `HOME`/`TEMP`), wrapper variables matching the credential/proxy/Node-injection deny patterns fail closed, and Windows keys are normalized case-insensitively so `Path`/`PATH` duplicates cannot bypass filtering.

...
| Measurement                  | WIT/Component (wasmtime 31)                          | IPC (JSON-lines subprocess)          |
| ---------------------------- | ---------------------------------------------------- | ------------------------------------ |
...
cargo test
cargo build --release --bins && cargo run --release --bin host
...
cd ../host && cargo run --release -- guest.component.wasm
```

See ADR 0034 for the decision and its revisit conditions.

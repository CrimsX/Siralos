...
- Slash commands: `/help`, `/status`, `/clear`, `/tools`, `/sandbox`, `/permissions`, `/git-status`, `/diff`, `/checkpoints`, `/undo`, `/commands`, `/cancel`, `/develop`, `/development-status`, `/exit`, `/godot`, `/godot-installations`, `/godot-project`, `/godot-doctor`, `/godot-probe`, `/godot-probe-status`, `/godot-knowledge`, `/godot-knowledge-refresh`, `/godot-api`, `/gdscript-check`, `/gdscript-diagnostics` (all are surfaces; see the capability notes below — `/undo` and `/commands` fail closed as unavailable, `/godot-probe` reports the recovery capability truthfully, `/godot-knowledge-refresh`, `/gdscript-check`, and `/gdscript-diagnostics` refuse before any approval while their execution is unavailable, and `/develop` refuses before any approval for a mutation while the change-set applier is unavailable)

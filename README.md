...
- Slash commands: `/help`, `/status`, `/clear`, `/tools`, `/sandbox`, `/permissions`, `/git-status`, `/diff`, `/checkpoints`, `/undo`, `/commands`, `/cancel`, `/exit`, `/godot`, `/godot-installations`, `/godot-project`, `/godot-doctor`, `/godot-probe`, `/godot-probe-status` (all are surfaces; see the capability notes below — `/undo` and `/commands` fail closed as unavailable, and `/godot-probe` reports the recovery capability truthfully)

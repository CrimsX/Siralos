...
Siralos is an independent, provider-neutral, interactive agent harness for programming and developing games with the Godot Engine.
...
In short: read-only workspace inspection is available; Git inspection is unavailable at this stage (the adapter requires Siralos-owned private run directories, whose creation and cleanup fail closed); workspace mutations, `/undo`, and development-command execution are intentionally unavailable; the GDScript development workflow (the bounded inspect → propose → approve → checkpoint → apply → parse → fresh-LSP → diagnose → repair loop) is implemented as contracts, change-set machinery, orchestration, and truthful reporting, but its execution gate — the exact change-set applier — fails closed as unavailable on every platform, so no approval for a mutation is ever requested at this stage and no checkpoint is ever created; Godot discovery and static project inspection are available, but Godot engine probing is intentionally unavailable.

Working today:

- Interactive terminal session (`npm run siralos`)
- Slash commands: `/help`, `/status`, `/clear`, `/tools`, `/sandbox`, `/permissions`, `/git-status`, `/diff`, `/checkpoints`, `/undo`, `/commands`, `/cancel`, `/develop`, `/plan`, `/development-status`, `/quality`, `/review-change`, `/exit`, `/godot`, `/godot-installations`, `/godot-project`, `/godot-doctor`, `/godot-probe`, `/godot-probe-status`, `/godot-knowledge`, `/godot-knowledge-refresh`, `/godot-api`, `/gdscript-check`, `/gdscript-diagnostics`, `/doctor`, `/siralos` (all are surfaces; see the capability notes below — `/undo` and `/commands` fail closed as unavailable, `/godot-probe` reports the recovery capability truthfully, `/godot-knowledge-refresh`, `/gdscript-check`, and `/gdscript-diagnostics` refuse before any approval while their execution is unavailable, `/develop` refuses before any approval for a mutation while the change-set applier is unavailable, and `/plan` is plan-only: read-only planning with zero workspace changes and zero mutation checkpoints)
...
- Deterministic fake provider (`deterministic-fake`) that requires no credentials and no network, with synthetic scenarios for read tools, the write-workflow scenarios (`create siralos-write-test`, `edit siralos-write-test`, `delete siralos-write-test`) that now exercise the fail-closed `unavailable` path, Git inspection (`git status`, `show working diff`, `show staged diff`, `show head diff`), development commands (`run npm check`, `run npm test`, `run node validation fixture`), which are refused as `unavailable`, and the development-loop scenarios (`develop fixture`, `develop fixture with repair`) that read a fixture script, propose an exact change set, and summarize the validated result or the truthful failure
...
Siralos is at the foundation stage. Do not expect it to develop games yet.
...
- For the Rust candidate implementation (Stage 3R): Rust stable as
  pinned in `rust-toolchain.toml` (via rustup; the toolchain lives at
  `~/.cargo/bin`). On Windows the GNU host target
  (`stable-x86_64-pc-windows-gnu`) is used and requires MinGW-w64
  (e.g. WinLibs) on `PATH`; on Linux/macOS the default host is used.
  See `docs/development/RUST_STYLE.md`.
...
This installs tooling at the root and links the workspace packages (`@siralos/core`, `@siralos/adapters`, `@siralos/cli`).

## Development setup

```bash
npm run build        # compile all workspaces with tsc -b
npm run siralos      # launch the interactive CLI
```
...
| `npm run test:godot`                  | Run live Godot probe conformance (opt-in; requires `SIRALOS_TEST_GODOT="<absolute-path>"`). At this stage it reports UNAVAILABLE loudly (probing fails closed) and never passes; it never modifies the user-supplied engine                                                                                                                                                                                                          |
| `npm run test:godot-recovery`         | Run live recovery-mode conformance (opt-in; requires `SIRALOS_TEST_GODOT="<absolute-path>"`). Verifies the fail-closed behavior truthfully (capability unavailable with a precise reason, nothing created, nothing executed); the live engine-isolation probe is always reported skipped while execution is unavailable and never passed                                                                                             |
| `npm run test:godot-development`      | Run live development-workflow conformance (opt-in; requires `SIRALOS_TEST_GODOT="<absolute-path>"`). Verifies the fail-closed behavior truthfully (capability unavailable with a precise reason, workflow preparation refuses before any approval, nothing created or executed, source untouched); the live development-loop isolation probe is always reported skipped while the change-set applier is fail-closed and never passed |
| `npm run test:godot-quality`          | Run live quality-stage conformance (opt-in; requires `SIRALOS_TEST_GODOT="<absolute-path>"`). Verifies the fail-closed behavior truthfully (workflow unavailable, so no quality stage, review, or validation command runs; nothing created or executed); the live quality-stage isolation probe is always reported skipped while the change-set applier is fail-closed and never passed                                              |
| `npm run check`                       | Run all non-mutating validation                                                                                                                                                                                                                                                                                                                                                                                                      |
| `npm run siralos`                     | Build and launch the interactive CLI                                                                                                                                                                                                                                                                                                                                                                                                 |
| `npm run siralos -- --sandbox-doctor` | Print sandbox diagnostics (add `--run-probes` to run fixed probes; exit 0 passed, 1 probe failure, 3 probes unavailable)                                                                                                                                                                                                                                                                                                             |
| `npm run siralos -- --godot-doctor`   | Print Godot discovery, selection, and cache diagnostics (exit 0–7; 0 success, 1 no valid engine, 2 selection failure, 3 sandbox unavailable, 4 probe failure, 5 identity mismatch, 6 degraded, 7 recovery capability unavailable when `--recovery-probe` was requested)                                                                                                                                                              |

## Sandbox configuration

User-level configuration lives at `~/.siralos/config.json`:
...
same-user process in the verify-to-launch window — so Siralos refuses instead
...
Siralos inspected 19 workspace entries.
...
Available tools:
  ...
  godot.inspect_engine - Inspect the selected Godot installation: exact version, edition, release channel, Siralos support classification, advertised and operationally verified capabilities, and the extension API dump fingerprint. Read-only; no project is opened or imported. (read-only, allowed)
...
Siralos explains its own installed behavior from host-owned metadata
instead of model memory:

- **`@siralos` self-reference** — built-in read-only documentation of the
  ...
  the `self.read` / `self.search` tools, `/siralos`, or `siralos --self`.
  It contains no secrets and has no mutation surface.
- **`siralos --doctor [area] [--json] [--report-safe]`** (interactive:
...
- The command catalog is the single source for the interactive command
  vocabulary and help; `/doctor` and `/siralos` are catalogued commands.
...
differential-verified subsystem port) is next.** The authoritative R1-R12
sequence is recorded in
[the Rust migration track](docs/development/RUST_MIGRATION.md). After
Stage 3R, the product proceeds to **Stage 4 milestone 1: Controlled
Runtime Execution** — a generic sandboxed, bounded supervised-process
boundary that produces structured runtime evidence without granting
unrestricted desktop or network access. Godot is its first later
specialization; it does not define the core runtime boundary. Stage 3 is
complete: the unified Godot-native development loop

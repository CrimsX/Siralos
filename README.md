...
- Slash commands: `/help`, `/status`, `/clear`, `/tools`, `/sandbox`, `/permissions`, `/git-status`, `/diff`, `/checkpoints`, `/undo`, `/commands`, `/cancel`, `/exit`, `/godot`, `/godot-installations`, `/godot-project`, `/godot-doctor`, `/godot-probe`, `/godot-probe-status`, `/godot-knowledge`, `/godot-knowledge-refresh`, `/godot-api`, `/gdscript-check`, `/gdscript-diagnostics` (all are surfaces; see the capability notes below — `/undo` and `/commands` fail closed as unavailable, `/godot-probe` reports the recovery capability truthfully, and `/godot-knowledge-refresh`, `/gdscript-check`, and `/gdscript-diagnostics` refuse before any approval while their execution is unavailable)
...
Godot executable discovery, validation, and static project detection are complete; read-only engine capability probing is intentionally unavailable at this stage (the probe runner reports `unavailable` and never spawns the executable), so no engine profile can be produced until an identity-bound launch primitive exists. The version-matched Godot knowledge surface (profile model, dump parser, index builder, search/lookup, CLI commands, ADR 0010) and the read-only GDScript diagnostic surface (check-only contracts, static preparation, normalization, approval machinery, CLI commands) are implemented as fail-closed surfaces: generation and check-only execution report typed `unavailable` outcomes on every platform and never spawn the engine. The bounded Godot GDScript LSP surface (session contracts, transport, normalization, session plan, CLI/tool commands, ADR 0011) is implemented as a fail-closed surface: session startup reports a typed `unavailable` outcome on every platform and never spawns the editor. The next narrow task after that is to integrate approved workspace edits with LSP-assisted GDScript development: inspect → propose edit → approve → checkpoint → apply → recreate/update the disposable language session → diagnose → repair, forming Solaris's first complete GDScript programming loop. Solaris does not open, import, execute, or run any Godot project at this stage. See `ROADMAP.md`.

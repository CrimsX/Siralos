...
- **Profiles**: `inspect` (read-only, no processes, no network — the default), `develop-offline` (workspace writes and process execution both require one-time approval, network denied, protected metadata paths, minimal environment, timeouts, output limits), and `validation-offline` (internal: command execution is bound to a read-only workspace regardless of the user profile — no command can execute at this stage because both runners fail closed as unavailable).

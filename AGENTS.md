...
- Current: Stage 3R R7 is Active. R7A behavior extraction and provider-protocol remediation are complete, and R7.1 — Provider Contract + Deterministic Fake Provider + Bounded Single Model Turn parity — is complete and evidence-backed (corpus version 12, 104 scenario files, 18 `provider-turn` scenarios at parity). The next implementation slice is R7.2 — Application Tool Loop parity — pending review/authorization; do not implement R7.2+ or R8+ work.

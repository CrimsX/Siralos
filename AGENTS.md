...
- Current: Stage 3R R7 is Active. R7A behavior extraction and provider-protocol remediation are complete, and R7.1 — Provider Contract + Deterministic Fake Provider + Bounded Single Model Turn parity — is complete and evidence-backed (corpus version 12, 104 scenario files, 18 `provider-turn` scenarios at parity; cancellation authority is Host-owned — providers receive only a read-only `CancellationSignal` and cannot mutate Host cancellation state). R7.2 � Application Tool Loop parity � is complete and evidence-backed (corpus version 13, 120 scenario files, 16 `tool-loop` scenarios at parity; cancellation authority is Host-owned � providers and Tools receive only a read-only `CancellationSignal`). Do not implement R7.3+ or R8+ work.
